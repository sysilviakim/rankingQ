...
test_that("plot_average_rank returns a ggplot object", {
  ...
  p <- plot_average_rank(result$results)
  expect_s3_class(p, "ggplot")
})

test_that("plot_average_rank with qoi_filter = NULL returns a ggplot object", {
  df <- data.frame(
  ...
  p <- plot_average_rank(df, qoi_filter = NULL)
  expect_s3_class(p, "ggplot")
})
